...
if [ "$FROM_JSON_PROMPT_LIST" ]; then
    added_params="--from-json-prompt-list $FROM_JSON_PROMPT_LIST"
fi
...
if [ "$SUBDIR_NAME" ]; then
    added_params="--outdir ${outdir}" # subdir name under the "outputs" dir to save files
fi
...
configs="${added_params} --ddim_steps ${ddim_steps} --plms --ckpt ${ckpt} --n_samples ${n_samples} --n_iter ${n_iter} --scale ${scale}"
